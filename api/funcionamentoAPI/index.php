<?php
  ...
  */
  ...
  $app->get('/funcionamentos', function ($request, $response, $args) {

    // importa do arquivo de configuracao
    require_once('../modulo/config.php');
    // import da controller de contatos, que fara a busca de dados
    require_once('../funcionamento/controller/controllerFuncionamentos.php');

    // solicita os dados para a controller
    if ($dados = listarFuncionamento()) {
  ...
  $app->get('/funcionamentos/{id}', function($request, $response, $args){
    ...
    require_once('../funcionamento/controller/controllerFuncionamentos.php');
    //solicita os dados para a controller
    if($dados = buscarFuncionamento($id))
    {
  ...
  $app->put('/funcionamentos/{id}', function($request, $response, $args){
    ...
    if(is_numeric($args['id']))
    {
      ...
      switch ($contentType[0]) {
        case 'application/json':
          ...
          $resposta = atualizarFuncionamento($arrayDados);
    ...
    {
      //retorna um erro que significa que o Funcionamento passou os dados errados
      return  $response   ->withStatus(404)
                          ->withHeader('Content-Type', 'application/json')
